...
public:
    ...
    // Drop stale microphone packets without allowing a producer to keep the
    // caller in an unbounded pop loop.
    size_t DropPacketsFromSendQueue();
...
private:
    ...
    std::atomic<uint32_t> voice_input_frames_{0};
    std::atomic<uint32_t> voice_encoded_packets_{0};
