        ...
        if (bits & (AS_EVENT_WAKE_WORD_RUNNING | AS_EVENT_AUDIO_PROCESSOR_RUNNING)) {
            ...
            if (ReadAudioData(data, 16000, samples)) {
                if (xEventGroupGetBits(event_group_) & AS_EVENT_AUDIO_PROCESSOR_RUNNING) {
                    const uint32_t frame = voice_input_frames_.fetch_add(1) + 1;
                    if (frame == 1 || frame % 100 == 0) {
                        ESP_LOGI(TAG, "Voice input: captured %lu 10 ms frames",
            ...
            if (opus_encoder_ != nullptr && task->pcm.size() == encoder_frame_size_) {
                ...
                if (ret == ESP_AUDIO_ERR_OK) {
                    ...
                    if (task->type == kAudioTaskTypeEncodeToSendQueue) {
                        ...
                        const uint32_t encoded = voice_encoded_packets_.fetch_add(1) + 1;
                        if (encoded == 1 || encoded % 20 == 0) {
                            ESP_LOGI(TAG, "Voice encoder: produced %lu Opus packets",
...
size_t AudioService::DropPacketsFromSendQueue() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
...
void AudioService::EnableVoiceProcessing(bool enable) {
    ...
    if (enable) {
        if (!InitializeAudioEngine()) {
            ESP_LOGE(TAG, "Voice processing cannot start: audio engine initialization failed");
            return;
        }
        voice_input_frames_.store(0);
        voice_encoded_packets_.store(0);
        ESP_LOGI(TAG, "Voice processing: resetting decoder and input pipeline");
        ResetDecoder();
        ...
        ESP_LOGI(TAG, "Voice processing: enabling audio engine");
        audio_engine_->EnableVoiceProcessing(true);
        xEventGroupSetBits(event_group_, AS_EVENT_AUDIO_PROCESSOR_RUNNING);
        ESP_LOGI(TAG, "Voice processing: input task enabled");
    } else {
        if (audio_engine_initialized_) {
