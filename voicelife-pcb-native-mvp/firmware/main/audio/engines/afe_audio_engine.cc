...
void AfeAudioEngine::HandleVoiceResult(const afe_fetch_result_t* result) {
    ...
    for (size_t i = 0; i < samples; ++i) {
        const int sample = result->data[i];
        const int magnitude = sample < 0 ? -sample : sample;
        if (magnitude > peak) {
            peak = magnitude;
        }
    }
    const uint32_t frame = voice_result_frames_.fetch_add(1) + 1;
    if (frame == 1 || frame % 32 == 0) {
        ESP_LOGI(TAG, "Voice AFE: frames=%lu vad=%d peak=%d",
                 static_cast<unsigned long>(frame), static_cast<int>(result->vad_state), peak);
    }
    if (vad_state_change_callback_) {
        if (result->vad_state == VAD_SPEECH && !is_speaking_) {
            is_speaking_ = true;
            ESP_LOGI(TAG, "Voice VAD: speech (frame=%lu peak=%d)",
                     static_cast<unsigned long>(frame), peak);
            vad_state_change_callback_(true);
        } else if (result->vad_state == VAD_SILENCE && is_speaking_) {
            is_speaking_ = false;
            ESP_LOGI(TAG, "Voice VAD: silence (frame=%lu peak=%d)",
                     static_cast<unsigned long>(frame), peak);
