...
bool Ota::LoadCachedWebsocketConfig() {
    ...
    ESP_LOGW(TAG, "Using cached WebSocket configuration after OTA check failure");
    return true;
}
...
esp_err_t Ota::CheckVersion() {
    ...
    config.event_handler = [](esp_http_client_event_t* event) -> esp_err_t {
        ...
        constexpr size_t kMaxOtaResponseSize = 16 * 1024;
        if (body->size() + static_cast<size_t>(event->data_len) > kMaxOtaResponseSize) {
            ESP_LOGE(TAG, "OTA response exceeds %u bytes",
                     static_cast<unsigned>(kMaxOtaResponseSize));
    ...
    esp_http_client_set_header(client, "Client-Id", board.GetUuid().c_str());
    if (has_serial_number_) {
    ...
    esp_http_client_set_header(client, "Accept-Language", Lang::CODE);
    ...
    if (request_error != ESP_OK) {
        ESP_LOGE(TAG, "ESP HTTP OTA request failed: %s", esp_err_to_name(request_error));
...
#if CONFIG_VOICELIFE_ENABLED
                // Unix time must stay in UTC for schedule comparisons. Set the
                // process timezone separately so localtime() still renders the
                // server-provided local offset on the display.
...
#if CONFIG_VOICELIFE_ENABLED
                ESP_LOGI(TAG, "System time synchronized in UTC");
#endif
            } else {
                ESP_LOGE(TAG, "Failed to set system time");
            }
        } else {
            ESP_LOGW(TAG, "Invalid server_time timestamp");
        }
    } else {
