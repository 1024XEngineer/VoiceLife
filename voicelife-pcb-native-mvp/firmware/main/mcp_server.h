...
using JsonToolCallback = std::function<ReturnValue(const cJSON*)>;
...
private:
    ...
    JsonToolCallback json_callback_;
...
public:
    ...
    inline bool is_json_tool() const { return static_cast<bool>(json_callback_); }
    ...
    std::string to_json() const {
        ...
        cJSON *input_schema = nullptr;
        if (!input_schema_json_.empty()) {
            input_schema = cJSON_Parse(input_schema_json_.c_str());
        }
    ...
    std::string CallReturnValue(ReturnValue return_value) {
        // 返回结果
    ...
    std::string Call(const PropertyList& properties) {
        return CallReturnValue(callback_(properties));
    }

    std::string CallJson(const cJSON* arguments) {
        return CallReturnValue(json_callback_(arguments));
    }
};
...
public:
    ...
    void AddJsonTool(const std::string& name, const std::string& description, const std::string& input_schema_json, JsonToolCallback callback);
