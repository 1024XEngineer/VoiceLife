...
export class InMemoryImUnitOfWork implements ImUnitOfWork, ImUnitOfWorkContext {
  ...
  public transaction<T>(
    work: (context: ImUnitOfWorkContext) => Promise<T>,
  ): Promise<T> {
    return work(this);
  }
  ...
  > {
    return Promise.resolve(
      this.channelRows.get(id as ChannelAccountId) ??
        ...
        [...this.inboundRows.values()].find(
          (event) => event.id === (id as InboundEventId),
        ) ??
  ...
  public findPendingByDisplayCodeHash(
    hash: string,
  ...
  public findExpiredPairingSessions(
    now: IsoDateTime,
  ): Promise<readonly PairingSession[]> {
    return Promise.resolve(
  ...
  public findActiveByIdentity(
    ...
    return Promise.resolve(
      [...this.bindingRows.values()].find(
        (binding) =>
          binding.externalIdentityId === externalIdentityId &&
  ...
  public findByExternalEvent(
    ...
    return Promise.resolve(
      this.inboundRows.get(inboundKey(channelAccountId, externalEventId)),
    );
  }
  ...
  public findByExternalMessage(
    ...
    const delivery = attempt === undefined
      ? undefined
      : this.deliveryRows.get(attempt.deliveryId);
    return Promise.resolve(
      delivery?.channelAccountId === channelAccountId ? delivery : undefined,
    );
  }
  ...
  public nextAttemptNo(deliveryId: DeliveryId): Promise<number> {
    const attempts = [...this.attemptRows.values()].filter(
  ...
  public listReceipts(deliveryId: DeliveryId): Promise<readonly DeliveryReceipt[]> {
    return Promise.resolve(
      [...this.receiptRows.values()].filter(
  ...
  public findByActionKeyHash(actionKeyHash: string): Promise<ImAction | undefined> {
    return Promise.resolve(
      [...this.actionRows.values()].find(
        (action) => action.actionKeyHash === actionKeyHash,
      ),
    );
  }
