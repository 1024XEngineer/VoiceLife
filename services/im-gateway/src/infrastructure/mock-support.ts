...
export class FixedClock implements Clock {
  public constructor(
    private value: IsoDateTime =
      "2026-08-03T00:00:00.000Z" as IsoDateTime,
  ) {}
...
export class MockExternalIdentityProtector
  implements ExternalIdentityProtector
{
...
export class InMemoryActionCommandStream implements ActionCommandStreamPort {
  ...
  public subscribe(
    subscription: ActionStreamSubscription,
    ...
    const start =
      subscription.lastEventId === undefined
        ...
        : Math.max(
            0,
            matching.findIndex(
              (command) => command.commandId === subscription.lastEventId,
            ) + 1,
          );
  ...
  public verify(token: string): Promise<ActionTokenClaims> {
    ...
    if (claims === undefined) {
      return Promise.reject(
        new ImGatewayError("action_not_found", "Action token is invalid"),
      );
    }
...
export class MockDeviceAuthenticationPort
  implements DeviceAuthenticationPort
{
...
async function* snapshot(
  commands: readonly ReminderActionCommand[],
): AsyncIterable<ReminderActionCommand> {
