...
export const DEVICE_API_ROUTES = {
  ...
  reminderActionResults:
    "/v1/devices/:deviceId/reminder-actions/:commandId/result",
...
export class DeviceIntentController {
  ...
  public async postReminderActionResult(input: {
    ...
    if (principal.deviceId !== input.deviceId) {
      throw new ImGatewayError(
        "invalid_transition",
        "Device principal does not match the result path",
      );
    }
  ...
  private async authenticateDevice(
    ...
    if (principal.deviceId !== expectedDeviceId) {
      throw new ImGatewayError(
        "invalid_transition",
        "Device principal does not match the intent body",
      );
    }
  }
  ...
  private assertIdempotencyKey(
    ...
    if (idempotencyKey !== businessEventId) {
      throw new ImGatewayError(
        "duplicate_event",
        "Idempotency-Key must equal the contract business event ID",
      );
    }
  }
}
...
export class ReminderActionStreamController {
  ...
  public async connect(input: {
    ...
    if (principal.deviceId !== input.deviceId) {
      throw new ImGatewayError(
        "invalid_transition",
        "Device token is not bound to the requested deviceId",
      );
    }
    await this.actions.expireDue();
    const expiresAt = await this.actions.resolveActionWindow(
      input.deviceId,
      input.reminderTriggerId,
    );
    const replay = await this.actions.replayPending(
      input.deviceId,
      input.reminderTriggerId,
      input.lastEventId,
    );
    ...
    return markCommandsProcessing(
      concatenateCommands(replay, live),
      this.actions,
    );
  }
}
