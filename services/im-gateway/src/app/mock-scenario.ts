...
export async function runMockNotificationScenario(): Promise<void> {
  ...
  const delivered = await gateway.application.deliveries.find(
    strongDelivery.deliveryId,
  );
  ...
  if (
    ...
    completedAction.result?.nextTriggerAt === undefined
  ) {
  ...
  const expiringToken = await gateway.application.actionUi.issue(
    expiringDelivery.deliveryId,
  );
  ...
  if (
    (await gateway.application.actions.find(expiringAction.commandId))?.status !==
    "expired"
  ) {
  ...
  if (
    (await gateway.application.pairing.find(anonymousPairing.session.id))?.status !==
    "expired"
  ) {
