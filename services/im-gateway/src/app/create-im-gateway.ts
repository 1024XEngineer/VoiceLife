...
import {
  DeviceIntentController,
  ReminderActionStreamController,
} from "../infrastructure/http/device-api.js";
import {
  FixedClock,
...
export function createImGateway(
  dependencies: ImGatewayDependencies,
): ImGatewayRuntime {
  ...
  const bindings = new DefaultBindingApplication(
    dependencies.unitOfWork,
    dependencies.clock,
  );
  const inboundEvents = new DefaultInboundEventApplication(
    dependencies.unitOfWork,
    dependencies.clock,
  );
  ...
  const deliveries = new DefaultDeliveryApplication(
    dependencies.unitOfWork,
    dependencies.ids,
    dependencies.clock,
  );
  const receipts = new DefaultReceiptApplication(
    dependencies.unitOfWork,
    dependencies.ids,
    dependencies.clock,
  );
  ...
  const actionUi = new DefaultActionUiApplication(
  ...
  const platformEvents = new DefaultPlatformEventApplication(
    inboundEvents,
    pairing,
    receipts,
    actionUi,
  ...
  return {
    application,
    deviceApi: new DeviceIntentController(
      notifications,
      actions,
