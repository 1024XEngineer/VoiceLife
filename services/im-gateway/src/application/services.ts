...
import type {
  ...
  NormalizedImEvent,
} from "../contracts/platform-events.js";
import type {
  ActionStatus,
  ChannelAccount,
  Delivery,
...
export class DefaultChannelAccountApplication
  implements ChannelAccountApplication
{
  public constructor(
  ...
  public register(command: RegisterChannelAccountCommand): Promise<ChannelAccount> {
    return this.unitOfWork.transaction(async (tx) => {
      ...
      const account: ChannelAccount = {
        ...
        ...(command.capabilityConfig === undefined
          ? {}
          : { capabilityConfig: command.capabilityConfig }),
        status: "active",
        createdAt: now,
        updatedAt: now,
      };
  ...
  public find(channelAccountId: ChannelAccountId): Promise<ChannelAccount | undefined> {
    return this.unitOfWork.transaction((tx) =>
      tx.channelAccounts.findById(channelAccountId),
    );
  }
...
export class DefaultPairingApplication implements PairingApplication {
  ...
  public async create(
    command: CreatePairingSessionCommand,
    ...
    const session: PairingSession = {
      ...
      ...(command.allowedPlatforms === undefined
        ? {}
        : { allowedPlatforms: command.allowedPlatforms }),
      status: "pending",
      expiresAt: this.clock.addMinutes(
        now,
        command.expiresInMinutes ?? DEFAULT_PAIRING_WINDOW_MINUTES,
      ),
      createdAt: now,
    };
  ...
  public async find(
    ...
    return this.unitOfWork.transaction((tx) =>
      tx.pairingSessions.findById(pairingSessionId),
    );
  }
  ...
  public async confirm(command: ConfirmPairingCommand): Promise<ImBinding> {
    ...
    const protectedIdentity = await this.identityProtector.protect(
    ...
    return this.unitOfWork.transaction(async (tx) => {
      ...
      if (
        session.allowedPlatforms !== undefined &&
        !session.allowedPlatforms.includes(account.platform)
      ) {
        throw new ImGatewayError(
          "capability_not_supported",
          "Platform is not allowed by the pairing session",
        );
      }
      ...
      if (userId === undefined) {
        throw new ImGatewayError(
          "binding_not_found",
          "Pairing confirmation requires an internal userId",
        );
      }

      let identity = await tx.identities.findByChannelAndHash(
        account.id,
  ...
  public expireDue(): Promise<number> {
    return this.unitOfWork.transaction(async (tx) => {
      const sessions = await tx.pairingSessions.findExpiredPairingSessions(
        this.clock.now(),
      );
...
export class DefaultBindingApplication implements BindingApplication {
  ...
  public findActiveByExternalIdentity(
    externalIdentityId: ExternalIdentityId,
  ): Promise<ImBinding | undefined> {
    return this.unitOfWork.transaction((tx) =>
      tx.bindings.findActiveByIdentity(externalIdentityId),
    );
  }
...
export class DefaultInboundEventApplication implements InboundEventApplication {
  ...
  public recordIfNew(
    ...
    return this.unitOfWork.transaction(async (tx) => {
      const duplicate = await tx.inboundEvents.findByExternalEvent(
...
export class DefaultPlatformEventApplication
  implements PlatformEventApplication
{
  public constructor(
  ...
  public async postEvent(
    event: NormalizedImEvent,
  ): Promise<void | ReminderActionCommand> {
  ...
  private async dispatch(
    ...
    if (event.type === "action.triggered") {
      return this.actionUi.execute(
        event.payload,
        event.externalIdentityId === undefined
          ? undefined
          : { actualIdentityId: event.externalIdentityId },
      );
    }
...
export class DefaultNotificationApplication implements NotificationApplication {
  ...
  public submitScheduleReceipt(
    intent: ScheduleReceiptIntent,
  ...
  private createDeliveries(input: {
    ...
    return this.unitOfWork.transaction(async (tx) => {
      ...
      const selected = input.deviceId === undefined
        ? bindings
        : bindings.filter(
            (binding) =>
              binding.deviceId === undefined || binding.deviceId === input.deviceId,
          );
      const deliveries: NotificationSubmission["deliveries"][number][] = [];
      const now = this.clock.now();

      for (const binding of selected) {
        const existing = await tx.deliveries.findByBusinessKey(
          input.businessEventId,
          binding.id,
          input.kind,
        );
        ...
        const delivery: Delivery = {
          ...
          presentationType,
          status: "pending",
          ...(input.actionStream === undefined
            ? {}
            : { expiresAt: input.actionStream.expiresAt }),
          createdAt: now,
          updatedAt: now,
        };
  ...
  public retryDeadLetter(deliveryId: DeliveryId): Promise<Delivery> {
    return this.unitOfWork.transaction(async (tx) => {
      ...
      if (
        ...
        throw new ImGatewayError(
          "invalid_transition",
          "Only dead-letter deliveries can be retried manually",
        );
      }
      const now = this.clock.now();
...
export class DefaultDeliveryDispatchApplication
  implements DeliveryDispatchApplication
{
  public constructor(
    private readonly unitOfWork: ImUnitOfWork,
    private readonly ids: IdGenerator,
  ...
  public async dispatch(deliveryId: DeliveryId): Promise<Delivery> {
    const target = await this.unitOfWork.transaction(async (tx) => {
      ...
      const identity = binding === undefined
        ? undefined
        : await tx.identities.findById(binding.externalIdentityId);
    ...
    const actionToken = target.delivery.expiresAt === undefined
      ? undefined
...
export class DefaultReceiptApplication implements ReceiptApplication {
  ...
  public record(receipt: NormalizedDeliveryReceipt): Promise<void> {
    return this.unitOfWork.transaction(async (tx) => {
      if (
        (await tx.deliveries.findReceiptByDedupeKey(receipt.dedupeKey)) !== undefined
      ) {
        return;
      }
      ...
      if (delivery === undefined) {
        throw new ImGatewayError(
          "delivery_not_found",
          "Delivery was not found for the platform message",
        );
      }
...
export class DefaultActionApplication implements ActionApplication {
  ...
  public prepareToken(deliveryId: DeliveryId): Promise<ActionTokenClaims> {
    return this.unitOfWork.transaction(async (tx) => {
      ...
      if (
        ...
        throw new ImGatewayError(
          "action_expired",
          "Delivery has no active strong-reminder action window",
        );
      }
      return {
  ...
  public inspectPrepared(claims: ActionTokenClaims): Promise<ActionUiView> {
    return this.unitOfWork.transaction(async (tx) => {
      ...
      const metadata = delivery === undefined
        ? undefined
        : readStrongReminderMetadata(delivery.semanticPayload);
      if (
        delivery === undefined ||
        metadata === undefined ||
  ...
  public async triggerPrepared(
    command: TriggerPreparedActionCommand,
  ): Promise<ReminderActionCommand> {
    const actionParams = validateReminderActionParams(
      command.actionType,
    ...
    const prepared = await this.unitOfWork.transaction(async (tx) => {
      ...
      if (existing !== undefined) {
        if (
          ...
          (command.actualIdentityId !== undefined &&
            command.actualIdentityId !== existing.expectedIdentityId)
        ) {
          throw new ImGatewayError(
            "action_not_found",
            "Action token was already used for another action",
          );
        }
        return { command: toCommand(existing), shouldDispatch: false };
      }
      const duplicateKey = await tx.actions.findByActionKeyHash(
        command.actionKeyHash,
      );
      ...
      const metadata = delivery === undefined
        ? undefined
        : readStrongReminderMetadata(delivery.semanticPayload);
      const binding = delivery === undefined
        ? undefined
        : await tx.bindings.findById(delivery.bindingId);
      if (
        delivery === undefined ||
        binding === undefined ||
        binding.status !== "active" ||
        (command.actualIdentityId !== undefined &&
          command.actualIdentityId !== binding.externalIdentityId) ||
        metadata === undefined ||
        ...
        throw new ImGatewayError(
          "action_expired",
          "Action token does not match an active delivery",
        );
      }
      const now = this.clock.now();
  ...
  private async recordResultAndClose(
    ...
    const updated = await this.unitOfWork.transaction(async (tx) => {
      ...
      if (
        ...
        throw new ImGatewayError(
          "invalid_transition",
          "Action result does not match command scope",
        );
      }
      if (
        ...
        ((action.actionType === "snooze" && result.nextTriggerAt === undefined) ||
          (action.actionType === "acknowledge" &&
            result.nextTriggerAt !== undefined))
      ) {
        throw new ImGatewayError(
          "invalid_transition",
      ...
      const status: ActionStatus = result.status === "retryable_failed"
        ? "pending"
        : result.status;
  ...
  public resolveActionWindow(
    ...
    return this.unitOfWork.transaction(async (tx) => {
      ...
      if (delivery?.expiresAt === undefined) {
        throw new ImGatewayError(
          "action_expired",
          "No active strong-reminder action window was found",
        );
      }
  ...
  public markProcessing(
    ...
    return this.unitOfWork.transaction(async (tx) => {
      ...
      if (
        ...
        throw new ImGatewayError(
          "invalid_transition",
          "Action cannot enter processing for this stream",
        );
      }
      await tx.actions.save({
        ...action,
  ...
  public findByOperationId(operationId: OperationId): Promise<ImAction | undefined> {
    return this.unitOfWork.transaction((tx) =>
      tx.actions.findByOperationId(operationId),
    );
  }
  ...
  public replayPending(
    ...
    return this.unitOfWork.transaction(async (tx) => {
      ...
      const start = after === undefined
        ? 0
        : Math.max(0, actions.findIndex((action) => action.id === after));
...
export class DefaultActionUiApplication implements ActionUiApplication {
  ...
  public async execute(
    ...
    return this.actions.triggerPrepared({
      ...
      ...(context?.actualIdentityId === undefined
        ? {}
        : { actualIdentityId: context.actualIdentityId }),
...
function toInboundEventType(
  type: NormalizedImEvent["type"],
): NormalizedImEvent["type"] {
  return type;
}
...
function validateReminderActionParams(
  ...
  if (action === "acknowledge") {
    if (params !== undefined) {
      throw new ImGatewayError(
        "invalid_transition",
        "acknowledge does not accept action params",
      );
    }
    return undefined;
  }
  if (
    ...
    throw new ImGatewayError(
      "invalid_transition",
      "snooze requires a positive integer params.minutes",
    );
  }
...
function advanceDeliveryStatus(
  ...
  receipt: NormalizedDeliveryReceipt["stage"],
): DeliveryStatus {
