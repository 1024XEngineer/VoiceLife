...
export function parseScheduleReceiptIntent(
  input: unknown,
  ...
  return {
    ...
    correlationId: requiredId<CorrelationId>(
      value,
      "correlationId",
      "body.correlationId",
    ),
...
export function parseNotificationIntent(input: unknown): NotificationIntent {
  ...
  const actions = value.actions.map((action, index) =>
    parseNotificationAction(action, `body.actions[${index}]`),
  );
  ...
  const base = {
    schemaVersion: contractVersion(value),
    businessEventId: requiredId<EventId>(
      value,
      "businessEventId",
    ...
    instanceId: requiredId<TimerInstanceId>(
      value,
      "instanceId",
      "body.instanceId",
    ),
    reminderTriggerId: requiredId<ReminderTriggerId>(
      value,
      "reminderTriggerId",
      "body.reminderTriggerId",
    ),
    content: {
...
export function parseReminderActionResult(input: unknown): ReminderActionResult {
  const value = objectAt(input, "body");
  const nextTriggerAt = optionalIsoDateTime(
    value,
    "nextTriggerAt",
    "body.nextTriggerAt",
  );
...
export function parseReminderActionIntent(input: unknown): ReminderActionIntent {
  ...
  const action = enumAt(
    value.action,
    ["acknowledge", "snooze"] as const,
    "body.action",
  );
  const params = value.params === undefined
    ? undefined
    : parseActionParams(value.params, "body.params");
...
function parseNotificationAction(
  ...
  const params = value.params === undefined
    ? undefined
    : parseActionParams(value.params, `${path}.params`);
...
function contractVersion(
  value: JsonObject,
): typeof DEVICE_CONTRACT_VERSION {
...
function invalid(path: string, expectation: string): never {
  throw new ImGatewayError(
    "invalid_contract",
    `Invalid device contract at ${path}: ${expectation}`,
  );
}
