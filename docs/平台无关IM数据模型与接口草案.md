...
业务服务完成状态变更后，在同一事务写入领域事件；领域事件分发器将同一事件并行交给硬件语音通道和 IM 辅助通道。查询不改变业务状态，但也产出一条业务结果事件，由同一分发器向两个通道发送同一份查询快照。
...
回调路径与投递路径分离：`回调处理器 → IM 辅助通道服务 → 业务服务`，其中 IM 辅助通道服务只完成去重、绑定与操作授权校验；`IM 辅助通道服务 → IM 投递工作者` 仅负责发送回执卡片。两条路径都不经过硬件语音通道。
...
| 领域事件分发器 | 将同一领域事件/查询结果并行分发给已订阅的通道 | 执行业务规则、等待任一通道完成。 |
| 硬件语音通道 | 订阅业务事件或查询结果并播报；接收硬件语音输入 | IM 的投递和回调处理。 |
| IM 辅助通道服务 | 创建 IM 回执、签发卡片操作授权、建立待投递项、处理回调去重 | 决定业务规则或直接访问平台 SDK。 |
| IM 投递工作者 | 读取待投递项、调用适配器、记录投递结果与重试 | 改写业务状态。 |
| 企业微信适配器 | 获取应用访问凭证、渲染/发送模板卡片、验签解密并标准化卡片回调 | 业务对象授权、业务操作执行。 |
| 回调处理器 | 接收企业微信回调，交给适配器验签解密、标准化并及时响应 | 直接执行未经授权的业务动作。 |
...
| `action_token` | 仅在短暂处理上下文使用；持久化时只保留其哈希或授权 ID。 |
...
`receiveCardAction` 的处理顺序必须是：平台事件去重 → 查找并验证绑定 → 验证 token 哈希、操作者、有效期和消费状态 → 调用业务服务 → 创建结果回执。业务服务仍要使用独立幂等键，不能只依赖回调去重。
...
1. 验签、解密和时间戳校验；
2. 尽快返回该平台要求的 ACK；
3. 解析卡片操作并写入 `im_inbound_events`；
4. 调用 `ImAssistService.receiveCardAction`；
5. 结果由新的 IM 回执和硬件语音通道分别发布。

企业微信回调使用公网 HTTPS 地址；该入口是唯一对外暴露的 IM 回调地址。回调验签和解密由企业微信适配器完成，后续收件箱和服务接口不依赖企业微信的原始请求格式。
...
2. `im_inbound_events` 按平台事件 ID 去重；重复回调只返回 ACK。
3. 服务校验该 token 未过期、未消费，且回调用户等于 `actor_user_id`。
4. 业务服务执行确认操作，生成新的业务事件。
