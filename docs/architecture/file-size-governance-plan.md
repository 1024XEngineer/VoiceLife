...
| P1 | `services/im-gateway/src/application/services.ts` | 1,754 -> 1,304 | 已拆出 `channel-account-application`、`pairing-application`、`binding-application`、`inbound-event-application` 与 `platform-event-application`；保留通知、投递、回执和动作的业务事务。后续优先拆通知受理或动作执行，但不拆散投递领取/失败/重试事务 | IM Gateway 全量 CI 通过（含 API 文档、类型与内存持久化契约）；PostgreSQL 契约须在可用服务环境补跑 |
...
`scripts/check_code_size.py` 已在 PR CI 和 `run_pre_submit_checks.sh` 中运行。它拒绝新增手写源码文件超过 500 行，并以 `config/code-size-baseline.json` 锁住当前超线的生产文件：后续 PR 不能让这些文件增长；若某个既有生产文件首次超过 500 行，也必须先拆分或由架构评审把其当前行数登记到基线。测试、生成物和第三方镜像不适用生产基线，但超过 800 行仍会给 Reviewer 警告。这样不会因历史债务一次性阻塞全仓库，也不会让债务继续扩大。`runtime.cc` 当前 693 行的例外理由是它仍是唯一的交互状态机协调点，下一次同时涉及启动、队列或证据处理的改动必须先拆出协调器。例外仅限生成物、第三方镜像、自动生成的契约 fixture 或经过架构 Review 的单一表驱动文件，PR 要写明原因、到期迁移计划和测试证据。
