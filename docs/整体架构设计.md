...
| [Issue #88：PCB VoiceLife MVP](https://github.com/1024XEngineer/XE6-15/issues/88) | PCB 作为事实源、Gateway 仅负责投递和动作回传、硬件验证与固件交付要求 |
...
| PCB 原生固件 MVP | 已形成本地固件基线 | [PCB 固件交付记录](./PCB-VoiceLife-MVP-固件交付记录.md) | 需继续接入真实 Gateway 与端到端硬件验收 |
...
| V0.5 | 2026-07-31 | 关联 Issue #88，并补充 PCB 原生固件 MVP 的当前交付记录 | 将架构目标与硬件侧固件验证状态对齐 |
