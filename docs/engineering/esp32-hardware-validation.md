...
> **当前板实读修正（2026-08-10）**：本文后续涉及当前 `/dev/cu.usbmodem5A840116301` 的操作，必须以 `partitions_voicelife_pcb_tutorial_actual.csv` 和 `/tmp/voicelife-linx-baseline-20260810-actual/manifest.json` 为准。直接读取到的布局是 `nvs@0x9000/0x6000`、`otadata@0xf000/0x2000`、`phy_init@0x11000/0x1000`、`ota_0@0x20000/0x3e0000`、`ota_1@0x400000/0x3e0000`、`voicelife@0x7e0000/0x200000`，没有 `assets` 或 `nvs_keys`。本文早期记录的 `ota_1@0x410000` 等地址属于另一份历史布局，禁止用于当前板写入。
...
| 分区 | 地址 | 大小 | 处理原则 |
| --- | ---: | ---: | --- |
| `nvs` | `0x009000` | `0x006000` | 保持原始内容，不主动初始化或加密 |
| `otadata` | `0x00f000` | `0x002000` | 切换测试槽前备份，测试结束恢复 |
| `ota_0` | `0x020000` | `0x3e0000` | 当前运行槽，不覆盖 |
| `ota_1` | `0x400000` | `0x3e0000` | 唯一允许写入测试 App 的槽 |
| `voicelife` | `0x7e0000` | `0x200000` | 业务数据，先完整备份再测试 |
