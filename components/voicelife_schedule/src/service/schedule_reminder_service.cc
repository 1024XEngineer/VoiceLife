...
ScheduleReminderService::ScheduleReminderService(ScheduleRepository& repository,
                                                 ScheduleReminderTaskRepository& reminder_repository,
    ...
    : repository_(repository), reminder_repository_(reminder_repository), schedule_service_(schedule_service),
      rule_service_(rule_service), timing_service_(&timing_service), speech_(speech), notification_(notification),
...
Status ScheduleReminderService::Start() {
    ...
    if (!reminders.ok()) return reminders.status;
    {
        ...
        running_ = true;
        sequence_ = std::chrono::duration_cast<std::chrono::microseconds>(
    ...
    for (const auto& schedule : *schedules.value) {
        if (schedule.status != ScheduleStatus::kActive || !schedule.start_time.has_value() ||
            *schedule.start_time <= Now() || schedules_with_tasks.contains(schedule.id)) continue;
...
void ScheduleReminderService::Stop() {
    ...
    if (all.ok()) for (const auto& task : *all.value) if (Pending(task) && task.timing_task_id) ids.push_back(*task.timing_task_id);
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (!running_) return;
        running_ = false;
        for (const auto& [rule, retry] : generation_retries_) { (void)rule; ids.push_back(retry.task_id); }
...
Status ScheduleReminderService::SynchronizeSchedule(ScheduleId schedule_id) {
    ...
    if (loaded.value->status != ScheduleStatus::kActive || !loaded.value->start_time.has_value() ||
        *loaded.value->start_time <= Now()) return Status::Ok();
...
Status ScheduleReminderService::CancelPendingTasks(ScheduleId schedule_id, std::optional<int64_t> except_task_id) {
    ...
    for (auto task : *tasks.value) {
        ...
        if (task.timing_task_id) {
            ...
            if (id && timing_service_->CancelTask({.task_id = *id, .on_result = {}}) == timing::CommandAcceptance::kUnavailable) {
                if (first_failure.ok()) first_failure = Status::Error(ErrorCode::kUnavailable, "提醒任务取消命令未被接收");
                continue;
            }
        }
...
Status ScheduleReminderService::SuspendRuleReminders(ScheduleRuleId rule_id) {
    ...
    std::optional<std::string> retry;
    {
        ...
        if (found != generation_retries_.end()) { retry = found->second.task_id; generation_retries_.erase(found); }
    }
    if (retry) {
        const auto id = timing::TaskId::Create(*retry);
        if (id && timing_service_->CancelTask({.task_id = *id, .on_result = {}}) == timing::CommandAcceptance::kUnavailable && first_failure.ok())
...
Result<ReminderActionResult> ScheduleReminderService::AcknowledgeRecentReminders() {
    ...
    for (const auto& task : *recent.value) {
        if (task.business_status != ScheduleReminderBusinessStatus::kWaitingAcknowledgement &&
            task.business_status != ScheduleReminderBusinessStatus::kExhausted) continue;
        chains.insert(task.chain_id);
        schedules.insert(task.schedule_id);
    }
    if (chains.empty()) return Result<ReminderActionResult>::Failure(ErrorCode::kNotFound, "最近 10 分钟内没有已触发的提醒");
    const auto all = reminder_repository_.FindAll();
...
Result<ReminderActionResult> ScheduleReminderService::SnoozeRecentReminders() {
    ...
    for (const auto& triggered : *recent.value) {
        if (triggered.business_status != ScheduleReminderBusinessStatus::kWaitingAcknowledgement &&
            triggered.business_status != ScheduleReminderBusinessStatus::kExhausted) continue;
        const bool has_pending_follow_up = std::any_of(
            all.value->begin(), all.value->end(), [&triggered](const auto& task) {
    ...
    if (chains.empty()) return Result<ReminderActionResult>::Failure(ErrorCode::kNotFound, "最近 10 分钟内没有可延迟的提醒");
    return Result<ReminderActionResult>::Success({.affected_count = static_cast<int>(chains.size())});
}
...
int64_t ScheduleReminderService::AllocateChainId() {
    std::lock_guard<std::mutex> lock(mutex_);
    if (chain_sequence_ == std::numeric_limits<int64_t>::max()) chain_sequence_ = 1;
    else ++chain_sequence_;
    return chain_sequence_;
}

std::string ScheduleReminderService::AllocateTaskId(std::string_view prefix) {
    std::lock_guard<std::mutex> lock(mutex_);
    if (sequence_ == std::numeric_limits<int64_t>::max()) sequence_ = 1; else ++sequence_;
...
Status ScheduleReminderService::RegisterPersistedTask(const ScheduleReminderTask& task) {
    ...
    const auto accepted = timing_service_->RegisterTask({
        .task_id = *id, .trigger_at = ToTriggerAt(task.trigger_at),
        .callback = [this, reminder_task_id = task.id](const timing::TaskId& fired, timing::TriggerAt) {
            HandleReminder(reminder_task_id, fired.Value());
        },
        .on_result = [this, reminder_task_id = task.id](timing::RegisterTaskResult result) {
            ...
            if (loaded.ok() && loaded.value->timer_status == ScheduleReminderTimerStatus::kPending) {
                auto failed = *loaded.value; failed.timer_status = ScheduleReminderTimerStatus::kFailed;
                failed.updated_at = Now(); (void)reminder_repository_.Update(failed);
            }
        }});
    return accepted == timing::CommandAcceptance::kAccepted ? Status::Ok()
...
void ScheduleReminderService::HandleReminder(int64_t reminder_task_id, std::string_view timing_task_id) {
    ...
    if (!loaded_task.ok() || !Pending(*loaded_task.value) || !loaded_task.value->timing_task_id ||
        *loaded_task.value->timing_task_id != timing_task_id) return;
    ...
    task.triggered_at = Now(); task.updated_at = Now();
...
void ScheduleReminderService::GenerateNextInstance(ScheduleRuleId rule_id, int prior_failure_count) {
    ...
    if (!result.status.ok()) { (void)ScheduleGenerationRetry(rule_id, prior_failure_count + 1); return; }
    { std::lock_guard<std::mutex> lock(mutex_); generation_retries_.erase(rule_id); }
...
Status ScheduleReminderService::ScheduleGenerationRetry(ScheduleRuleId rule_id, int failures) {
    ...
    { std::lock_guard<std::mutex> lock(mutex_); generation_retries_[rule_id] = {.task_id = value, .failure_count = failures}; }
    const auto accepted = timing_service_->RegisterTask({
        .task_id = *id, .trigger_at = ToTriggerAt(Now() + RetryDelay(failures)),
        .callback = [this, rule_id, value](const timing::TaskId& fired, timing::TriggerAt) {
            if (fired.Value() != value) return;
            int failures = 0;
            { std::lock_guard<std::mutex> lock(mutex_); auto found = generation_retries_.find(rule_id);
              if (found == generation_retries_.end() || found->second.task_id != value) return;
              failures = found->second.failure_count; generation_retries_.erase(found); }
            GenerateNextInstance(rule_id, failures);
        },
        .on_result = [this, rule_id, value](timing::RegisterTaskResult result) {
            if (result != timing::RegisterTaskResult::kDuplicate) return;
            std::lock_guard<std::mutex> lock(mutex_); auto found = generation_retries_.find(rule_id);
            if (found != generation_retries_.end() && found->second.task_id == value) generation_retries_.erase(found);
        }});
...
bool ScheduleReminderService::IsRunning() const { std::lock_guard<std::mutex> lock(mutex_); return running_; }
}  // namespace voicelife::schedule
