...
voicelife::Status SparkBotLvglRenderer::SetupUI() {
    ...
    // 中央 emoji 舞台：缩小到半高视口内，GIF 源仍可保持官方 96x96。
    ...
    lv_obj_set_style_text_font(emoji_label, &font_material_symbols_20_4, 0);
    ...
    lv_obj_set_style_text_font(camera_label, &font_material_symbols_14_1, 0);
    lv_obj_set_style_text_color(camera_label, lv_color_hex(0x6DD8E8), 0);
    lv_obj_set_style_margin_left(camera_label, 2, 0);
...
voicelife::Status SparkBotLvglRenderer::Render(const voicelife::voice::DisplaySnapshot& snapshot) {
    ...
    ESP_LOGI(
        ...
        "content_xywh=%d,%d,%d,%d scroll_mode=horizontal scroll_duration_ms=%u common_font=%d status=%.*s content=%.*s",
