#include "schema/migrations/v007_add_reminder_action_state.h"

namespace voicelife::storage_sqlite::schema::migrations {
namespace {

constexpr char kAddReminderActionState[] = R"sql(
ALTER TABLE schedule_reminder_task RENAME TO schedule_reminder_task_v006;
...
INSERT INTO schedule_reminder_task (
    id, schedule_id, chain_id, attempt, timing_task_id, trigger_at, business_status,
    timer_status, triggered_at, created_at, updated_at
)
SELECT id, schedule_id, chain_id, attempt, timing_task_id, trigger_at, business_status,
       timer_status, triggered_at, created_at, updated_at
FROM schedule_reminder_task_v006;
DROP TABLE schedule_reminder_task_v006;
...
Status ApplyV007AddReminderActionState(SqliteDatabase& database) { return database.Execute(kAddReminderActionState); }
