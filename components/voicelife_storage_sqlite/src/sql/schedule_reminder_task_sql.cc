...
(schedule_id, chain_id, attempt, timing_task_id, trigger_at, business_status, timer_status,
 triggered_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
)sql";
const char kFindAllScheduleReminderTasks[] = R"sql(
SELECT id, schedule_id, chain_id, attempt, timing_task_id, trigger_at, business_status, timer_status,
       triggered_at, created_at, updated_at FROM schedule_reminder_task ORDER BY trigger_at, id
)sql";
const char kFindScheduleReminderTaskById[] = R"sql(
SELECT id, schedule_id, chain_id, attempt, timing_task_id, trigger_at, business_status, timer_status,
       triggered_at, created_at, updated_at FROM schedule_reminder_task WHERE id = ?
)sql";
const char kFindScheduleReminderTasksBySchedule[] = R"sql(
SELECT id, schedule_id, chain_id, attempt, timing_task_id, trigger_at, business_status, timer_status,
       triggered_at, created_at, updated_at FROM schedule_reminder_task WHERE schedule_id = ? ORDER BY chain_id, attempt, id
)sql";
const char kFindTriggeredScheduleReminderTasks[] = R"sql(
SELECT id, schedule_id, chain_id, attempt, timing_task_id, trigger_at, business_status, timer_status,
       triggered_at, created_at, updated_at FROM schedule_reminder_task
