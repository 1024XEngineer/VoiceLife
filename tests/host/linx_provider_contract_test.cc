...
int main() {
    ...
    Check(formats.ok() && formats.value->capture.sample_rate_hz == 16000 &&
              formats.value->playback.sample_rate_hz == 24000 &&
              formats.value->playback.frame_duration_ms == 60,
    ...
    Check(events.back().kind == voicelife::voice::VoiceEventKind::kDisconnected,
          "物理断线必须向会话上报生命周期事件");
    Check(provider.SendAudio(uplink).code == ErrorCode::kUnavailable,
          "断线后必须立即阻断音频上行");
    provider.SetGeneration(8);
    transport.EmitConnected();
    Check(transport.texts.size() == 6 &&
              transport.texts.back().find("\"type\":\"hello\"") != std::string::npos,
          "自动重连后必须只补发一次 hello");
    Check(events.back().kind == voicelife::voice::VoiceEventKind::kConnected &&
              events.back().generation == 8,
