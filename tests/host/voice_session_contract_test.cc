...
int main() {
    ...
    connect_failure_provider.connect_result =
        Status::Error(ErrorCode::kUnavailable, "Provider 连接失败");
    voicelife::voice::VoiceSession connect_failure(
        connect_failure_input, connect_failure_output, connect_failure_provider);
    ...
    capture_failure_provider.stop_result =
        Status::Error(ErrorCode::kUnavailable, "远端采集回滚失败");
    voicelife::voice::VoiceSession capture_failure(
        capture_failure_input, capture_failure_output, capture_failure_provider);
    ...
    voicelife::voice::VoiceSession stop_capture_failure(
        stop_capture_failure_input, stop_capture_failure_output, stop_capture_failure_provider);
    Check(stop_capture_failure.Start(Config()).ok() && stop_capture_failure.BeginCapture().ok(),
          "停止采集失败用例应先进入 capturing");
    stop_capture_failure_provider.stop_result =
        Status::Error(ErrorCode::kUnavailable, "远端停止失败");
    ...
    disconnect_failure_provider.disconnect_result =
        Status::Error(ErrorCode::kUnavailable, "Provider 断开失败");
    voicelife::voice::VoiceSession disconnect_failure(
        disconnect_failure_input, disconnect_failure_output, disconnect_failure_provider);
